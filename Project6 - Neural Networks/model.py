...
DB = data.load_mnist("D:/arti_intel/Assignments/Project6/project6/data/not_mnist", batch_size=BATCH_SIZE)
...
class Model(object):
    def __init__(self, learning_rate):
        ...
        self.param = {
           # "name": nn.parameter(blah, blah) 
            "w": nn.parameter(np.random.uniform(-0.01, 0.01, [n_features,150])),
            "b": nn.parameter.zeros([150]) ,
            "w1" : nn.parameter(np.random.uniform(-0.01, 0.01,[150, 200])),
            
            "b1" : nn.parameter.zeros([200]),
            "w2" : nn.parameter(np.random.uniform(-0.01, 0.01,[200, n_classes])),
    ...
    def get_loss(self, x, y):
        ...
        loss, probs = nn.cross_entropy_loss(logits=logits, labels=y)
        #######################################################################
    ...
    try:
        ...
        for epoch in range(1, MAX_EPOCHS+1):
            ...
            for step, (x, y) in enumerate(DB.train, 1):            
                ...
                "*** YOUR CODE HERE ***"
                logits = model.forward_propagation(x)
                prob , loss =model.get_loss(x, y)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
                accuracy = model.get_correct_predictions(prob, y)
                global_step += 1
                #######################################################################
                logger.log(loss.item(), accuracy.item(), global_step, True)
